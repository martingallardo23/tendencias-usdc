...
export const FrequencySlider = () => {
  const [setTimeType] = useStore((state) => [state.setTimeType]);
    ...
    <div className="flex flex-row rounded-full border pl-4 md:pl-5 pr-1 md:pr-2 py-0.5 md:py-1 bg-[var(--sliders-bg)] gap-3 items-center slider">
      ...
      <Tabs defaultValue="1h" className="flex flex-row bg-none">
        <TabsList className="flex flex-row gap-1">
          ...
          <TabsTrigger value="1h" onClick={() => setTimeType("1h")}>
            1h
          </TabsTrigger>
          <TabsTrigger value="12h" onClick={() => setTimeType("12h")}>
            {" "}
            12h{" "}
          </TabsTrigger>
          <TabsTrigger value="24h" onClick={() => setTimeType("24h")}>
            {" "}
            24h{" "}
          </TabsTrigger>
        </TabsList>
  ...
  return (
    <div className="flex flex-row rounded-full border pl-4 md:pl-5 pr-1 md:pr-2 py-0.5 md:py-1 bg-[var(--sliders-bg)] gap-3 items-center slider">
      ...
      <Tabs defaultValue="7d" className="flex flex-row bg-none">
        <TabsList className="flex flex-row gap-1">
          <TabsTrigger value="7d" onClick={() => setTimeFrame("7d")}>
            7d
          </TabsTrigger>
          {daysSinceFirst > 14 ? (
            <TabsTrigger value="14d" onClick={() => setTimeFrame("14d")}>
              14d
            </TabsTrigger>
          ) : null}
          {daysSinceFirst > 30 ? (
  ...
  return (
    <div className="flex flex-row rounded-full border pl-4 md:pl-5 pr-1 md:pr-2 py-0.5 md:py-1  bg-[var(--sliders-bg)] gap-3 items-center slider">
      ...
      <Tabs defaultValue="bids" className="flex flex-row bg-none">
        <TabsList className="flex flex-row gap-1">
          <TabsTrigger value="bids" onClick={() => setPriceType("bid")}>
            Compra
          </TabsTrigger>
          <TabsTrigger value="ask" onClick={() => setPriceType("ask")}>
            Venta
          </TabsTrigger>
          <TabsTrigger value="spread" onClick={() => setPriceType("spread")}>
            {" "}
            Spread{" "}
          </TabsTrigger>
        </TabsList>
...
export const ChartTypeSlider = () => {
  ...
  return (
    <div className="flex flex-row rounded-full border pl-4 md:pl-5 pr-1 md:pr-2 py-0.5 md:py-1  bg-[var(--sliders-bg)] gap-3 items-center slider">
      ...
      <Tabs defaultValue="bids" className="flex flex-row bg-none">
        <TabsList className="flex flex-row gap-1">
          <TabsTrigger value="bids" onClick={() => setChartType("average")}>
            Promedio
          </TabsTrigger>
          <TabsTrigger value="ask" onClick={() => setChartType("exchange")}>
            Todos
          </TabsTrigger>
        </TabsList>
      </Tabs>
...
export const ExchangeList = () => {
  ...
  function handleExchangeVisibilityChange() {
    ...
    d3.select(`#lineOverlay${exchange}`).style(
      "display",
    ...
    return (
      <div className="text-white w-[80%] flex gap-3 items-center justify-center flex-wrap mt-4 exchange-list">
        {Object.keys(exchangesVisible)
          ...
          .map((exchange) => (
            <Toggle
              className=" flex flex-row items-center gap-2 rounded-full data-[state=on]:bg-[var(--green-focus)]
              data-[state=on]:text-[var(--sliders-bg)] data-[state=off]:text-[var(--sliders-bg)] hover:bg-[#1B9D5E]
              hover:text-black data-[state=on]:hover:outline "
              name={exchange}
              defaultPressed={exchangesVisible[exchange]}
              key={exchange}
              onPressedChange={handleExchangeVisibilityChange}
              onMouseOver={(event) => hoverOver(event)}
              onMouseOut={(event) => hoverOut(event)}
            >
              <div
                className="w-2 h-2 rounded-full"
                style={{
                  backgroundColor: `var(--${exchange})`,
                  outline: "2px solid var(--sliders-bg)",
                }}
              ></div>
